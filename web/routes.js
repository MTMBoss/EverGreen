...
const { getActiveMembers } = require("../attendance/attendanceRepository");
...
function createWebRouter(client) {
  ...
  router.get(
    ...
    asyncHandler(async (_req, res) => {
      ...
      const dayView = await getDayView(today);
  ...
  router.options("/api/roster", allowRosterApiCors, (_req, res) => {
    res.status(204).send();
  });

  router.options("/api/roster/sync", allowRosterApiCors, (_req, res) => {
    res.status(204).send();
  });
  ...
  router.get(
    "/api/roster",
    allowRosterApiCors,
    requireRosterApiToken,
    asyncHandler(async (_req, res) => {
      const members = await getActiveMembers();
      res.json(formatRosterApiResponse(members));
    })
  );
  ...
  router.get(
    ...
    asyncHandler(async (req, res) => {
      ...
      const entries = dayView.entries.map(entry => {
        ...
        const declaredSlotCount = countDeclaredSlots(declaredSchedule);
        const actualSlotCount = countDeclaredSlots(actualSlots);
        ...
        return {
          ...
          syncLabel:
            mismatchCount === 0
              ? "Allineato"
              : declaredSlotCount > actualSlotCount
...
function allowRosterApiCors(_req, res, next) {
...
function requireRosterApiToken(req, res, next) {
  const expected = process.env.ROSTER_API_TOKEN || process.env.WEB_ADMIN_TOKEN || "";
  const authorization = String(req.headers.authorization || "");
  const bearerToken = authorization.startsWith("Bearer ")
    ? authorization.slice("Bearer ".length).trim()
    : "";
  const headerToken = String(req.headers["x-roster-token"] || "").trim();
  const queryToken = String(req.query.token || "").trim();

  if (expected && [bearerToken, headerToken, queryToken].includes(expected)) {
    return next();
  }
...
function formatRosterApiResponse(members) {
  return {
    version: 1,
    syncedAt: new Date().toISOString(),
    count: members.length,
    roster: members.map(member => {
      const roles = Array.isArray(member.tracked_roles) ? member.tracked_roles : [];
      const name =
        member.ingame_name ||
        member.nickname ||
        member.display_name ||
      ...
      return {
        ...
        tracked_roles: roles,
        roles: roles.map(role => role.roleName || role.name).filter(Boolean),
        notes: roles.length
          ? `Ruoli Discord: ${roles.map(role => role.roleName || role.name).filter(Boolean).join(", ")}`
          : "Roster Discord",
      };
    }),
  };
