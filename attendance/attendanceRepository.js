...
const deactivateMissingStmt = db.prepare(`
  UPDATE members
  SET active = 0
  WHERE discord_user_id NOT IN (SELECT value FROM json_each(?))
`);

const getMemberByDiscordIdStmt = db.prepare(`
  SELECT * FROM members
  WHERE discord_user_id = ?
  LIMIT 1
`);

const getActiveMembersStmt = db.prepare(`
  SELECT *
  FROM members
  WHERE active = 1
...
const insertAttendanceDayStmt = db.prepare(`
  INSERT OR IGNORE INTO attendance_days (day_date)
  VALUES (?)
`);

const getAttendanceDayStmt = db.prepare(`
  SELECT *
...
const ensureEntriesForDayStmt = db.prepare(`
...
const upsertAttendanceEntryStmt = db.prepare(`
...
const getAttendanceEntryByMemberAndDateStmt = db.prepare(`
  SELECT
    ae.*,
    m.discord_user_id,
    m.nickname,
...
const getAttendanceForDateStmt = db.prepare(`
  SELECT
    ae.id,
    ae.member_id,
    ae.slot_21_22,
...
const getAttendanceSummaryForDateStmt = db.prepare(`
  SELECT
...
const getMonthSummaryStmt = db.prepare(`
  SELECT
    ad.day_date,
    COUNT(*) AS total_members,
...
const syncMembersTransaction = db.transaction((members, syncedAt) => {
    const activeIds = [];
    ...
    deactivateMissingStmt.run(JSON.stringify(activeIds));
});

function syncTrackedMembers(members, syncedAt) {
    syncMembersTransaction(members, syncedAt);
}

function getMemberByDiscordId(discordUserId) {
    const row = getMemberByDiscordIdStmt.get(discordUserId);
    if (!row) return null;
...
function getActiveMembers() {
    return getActiveMembersStmt.all().map(row => ({
        ...row,
        tracked_roles: safeParseRoles(row.tracked_roles_json),
    }));
}
...
function ensureAttendanceDay(date, nowIso) {
    insertAttendanceDayStmt.run(date);
    const day = getAttendanceDayStmt.get(date);
    ensureEntriesForDayStmt.run(day.id, nowIso);
    return day;
}
...
function getAttendanceEntryByMemberAndDate(date, discordUserId) {
    const row = getAttendanceEntryByMemberAndDateStmt.get(date, discordUserId);
    if (!row) return null;
...
function getAttendanceForDate(date) {
    return getAttendanceForDateStmt.all(date).map(row => ({
        ...row,
        tracked_roles: safeParseRoles(row.tracked_roles_json),
    }));
}
...
function getAttendanceSummaryForDate(date) {
    return (
        getAttendanceSummaryForDateStmt.get(date) || {
            total_members: 0,
            slot_21_22_count: 0,
            slot_22_23_count: 0,
            slot_23_00_count: 0,
...
function getMonthSummary(startDate, endDate) {
    return getMonthSummaryStmt.all(startDate, endDate);
}
